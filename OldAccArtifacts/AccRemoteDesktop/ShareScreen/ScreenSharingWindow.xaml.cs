...
namespace ShareScreen
{
    ...
    public partial class ScreenSharingWindow : Window, INotifyPropertyChanged
    {
        ...
        private async void ImageHolder_MouseMove(object sender, MouseEventArgs e)
        {
            System.Drawing.Point p = new System.Drawing.Point(
                (int)(e.GetPosition(ss).X * (OriginalWidth / ss.ActualWidth)),
                (int)(e.GetPosition(ss).Y * (OriginalHeight / ss.ActualHeight))
            );
            await Communicator.Instance.ProduceMouseMove(p.X, p.Y, _host);
        }

        private async void ImageHolder_MouseDown(object sender, MouseButtonEventArgs e)
        {
            _mouseUp = false;
            if (e.ClickCount == 2)
            {
                await Communicator.Instance.Produce(
                    new InputDataComm()
                    {
        ...
        private async void ImageHolder_MouseUp(object sender, MouseButtonEventArgs e)
        {
            _mouseUp = true;
